...
#define NEXTIS3PLUS(c, x, d, y, e, z, a)                                                                                                             \
    {                                                                                                                                                \
        if (NextChar == (c))                                                                                                                         \
        {                                                                                                                                            \
            LEXER_INC(Lexer);                                                                                                                        \
...
static unsigned char LexUnEscapeCharacterConstant(const char** From, unsigned char FirstChar, int Base);
static char* LexUnEscapeCharacter(const char** From, const char* End);
char* LexUnEscapeUnicode16Character(const char** From, unsigned char FirstChar);
char* LexUnEscapeUnicode32Character(const char** From, unsigned char FirstChar);
static enum LexToken LexGetStringConstant(Picoc* pc, struct LexState* Lexer, struct Value* Value, char EndChar);
static enum LexToken LexGetCharacterConstant(Picoc* pc, struct LexState* Lexer, struct Value* Value);
...
static enum LexToken LexScanGetToken(Picoc* pc, struct LexState* Lexer, struct Value** Value);
static int LexTokenSize(enum LexToken Token);
static void* LexTokenize(Picoc* pc, struct LexState* Lexer, int* TokenLen);
static enum LexToken LexGetRawToken(struct ParseState* Parser, struct Value** Value, int IncPos);
...
static struct ReservedWord ReservedWords[] = {
    ...
    {"#define", TokenHashDefine}, {"#else", TokenHashElse}, {"#endif", TokenHashEndif}, {"#if", TokenHashIf}, {"#ifdef", TokenHashIfdef},
    {"#ifndef", TokenHashIfndef}, {"#include", TokenHashInclude}, {"auto", TokenAutoType}, {"break", TokenBreak}, {"case", TokenCase},
    {"char", TokenCharType}, {"continue", TokenContinue}, {"default", TokenDefault}, {"delete", TokenDelete}, {"do", TokenDo},
    {"double", TokenDoubleType}, {"else", TokenElse}, {"enum", TokenEnumType}, {"extern", TokenExternType}, {"float", TokenFloatType},
    {"for", TokenFor}, {"goto", TokenGoto}, {"if", TokenIf}, {"int", TokenIntType}, {"long", TokenLongType}, {"new", TokenNew},
    {"register", TokenRegisterType}, {"return", TokenReturn}, {"short", TokenShortType}, {"signed", TokenSignedType}, {"sizeof", TokenSizeof},
    {"static", TokenStaticType}, {"struct", TokenStructType}, {"switch", TokenSwitch}, {"typedef", TokenTypedef}, {"union", TokenUnionType},
    {"unsigned", TokenUnsignedType}, {"void", TokenVoidType}, {"while", TokenWhile}};
...
enum LexToken LexGetNumber(Picoc* pc, struct LexState* Lexer, struct Value* Value)
{
    ...
    // If it doesn't fit in an int, then it's a long
    ...
    if (IsLong && IsUnsigned)
    {
        ...
        ResultToken = TokenUnsignedLongConstant;
    }
    else if (IsLong)
    {
        Value->Typ  = &pc->LongType;
        ResultToken = TokenLongConstant;
    }
    else if (IsUnsigned)
    {
        Value->Typ  = &pc->UnsignedIntType;
        ResultToken = TokenUnsignedIntegerConstant;
    }
    else
    {
        ...
        else
        {
            ...
            switch (ThisChar)
            {
                ...
                case 't':
                    ...
                    break;
                case 'u':
                    strcat(Ret, LexUnEscapeUnicode16Character(From, ThisChar));
                    break;
                case 'U':
                    strcat(Ret, LexUnEscapeUnicode32Character(From, ThisChar));
                    break;
                case 'v':
                    Ret[0] = '\v';
                    break;
                ...
                case '2':
                case '3':
                    Ret[0] = LexUnEscapeCharacterConstant(From, ThisChar, 8);
                    break;
                case 'x':
                    Ret[0] = LexUnEscapeCharacterConstant(From, '0', 16);
                    break;
                default:
                    Ret[0] = ThisChar;
                    break;
...
int LexTokenSize(enum LexToken Token)
{
    switch (Token)
    {
        ...
        case TokenUnsignedIntegerConstant:
            return sizeof(int);
        case TokenLongConstant:
        case TokenUnsignedLongConstant:
            return sizeof(long);
...
void LexHashIf(struct ParseState* Parser)
{
    ...
    if (Token != TokenCharacterConstant && Token != TokenIntegerConstant && Token != TokenUnsignedIntegerConstant && Token != TokenLongConstant &&
        Token != TokenUnsignedLongConstant)
        ProgramFail(Parser, "value expected");
    ...
    else
    {
        ...
        else
        {
            ...
            for (ILine = pc->InteractiveCurrentLine->Next;
                 ILine != NULL && (EndParser->Pos < &ILine->Tokens[0] || EndParser->Pos >= &ILine->Tokens[ILine->NumBytes]); ILine = ILine->Next)
...
{
    while (pc->InteractiveHead != NULL &&
           !(Parser->Pos >= &pc->InteractiveHead->Tokens[0] && Parser->Pos < &pc->InteractiveHead->Tokens[pc->InteractiveHead->NumBytes]))
    {
