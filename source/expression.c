...
#define ASSIGN_FP_OR_INT(value)                                                                                                                      \
    ...
    {                                                                                                                                                \
        ResultFP = ExpressionAssignFP(Parser, BottomValue, value, false);                                                                            \
    }                                                                                                                                                \
    else                                                                                                                                             \
    {                                                                                                                                                \
...
static double ExpressionAssignFP(struct ParseState* Parser, struct Value* DestValue, double FromFP, int After);
...
static void ExpressionPushInt(struct ParseState* Parser, struct ExpressionStack** StackTop, int IntValue);
static void ExpressionPushUnsignedInt(struct ParseState* Parser, struct ExpressionStack** StackTop, unsigned int IntValue);
static void ExpressionPushLong(struct ParseState* Parser, struct ExpressionStack** StackTop, long IntValue);
static void ExpressionPushUnsignedLong(struct ParseState* Parser, struct ExpressionStack** StackTop, unsigned long IntValue);
...
static void ExpressionAssignToPointer(
    struct ParseState* Parser, struct Value* ToValue, struct Value* FromValue, const char* FuncName, int ParamNo, int AllowPointerCoercion);
static void ExpressionQuestionMarkOperator(
    struct ParseState* Parser, struct ExpressionStack** StackTop, struct Value* BottomValue, struct Value* TopValue);
...
static void ExpressionInfixOperator(
...
static void ExpressionStackPushOperator(
    struct ParseState* Parser, struct ExpressionStack** StackTop, enum OperatorOrder Order, enum LexToken Token, int Precedence);
    ...
    while (StackTop != NULL)
    {
        if (StackTop->Order == OrderNone)
        {
            ...
            switch (StackTop->Val->Typ->Base)
            {
                ...
                case TypeUnsignedLong:
                    ...
                    break;
                case TypeFloat:
                    printf("%f:float", StackTop->Val->Val->Float);
                    break;
                case TypeDouble:
                    printf("%f:double", StackTop->Val->Val->Double);
                    break;
                case TypeFunction:
        ...
        else
        {
            ...
            printf("op='%s' %s %d", OperatorPrecedence[(int)StackTop->Op].Name,
                (StackTop->Order == OrderPrefix) ? "prefix" : ((StackTop->Order == OrderPostfix) ? "postfix" : "infix"), StackTop->Precedence);
...
long ExpressionCoerceInteger(struct Value* Val)
{
    switch (Val->Typ->Base)
    {
        ...
        case TypeFloat:
            return (long)Val->Val->Float;
        case TypeDouble:
            return (long)Val->Val->Double;
        default:
            return 0;
    }
}
...
float ExpressionCoerceFloat(struct Value* Val)
{
    switch (Val->Typ->Base)
    {
        ...
        case TypeFloat:
            return (float)Val->Val->Float;
        case TypeDouble:
            return (float)Val->Val->Double;
        default:
            return 0.0;
    }
}
...
double ExpressionAssignFP(struct ParseState* Parser, struct Value* DestValue, double FromFP, int After)
{
    double Result;
    ...
    if (After)
        Result = ExpressionCoerceDouble(DestValue);
    else
        Result = FromFP;
    ...
    switch (DestValue->Typ->Base)
    {
        case TypeFloat:
            ...
            break;
        case TypeDouble:
            DestValue->Val->Double = (double)FromFP;
            break;
        default:
            break;
    }
...
{
    struct ExpressionStack* StackNode = VariableAlloc(Parser->pc, Parser, sizeof(*StackNode), false);
...
void ExpressionStackPushDereference(struct ParseState* Parser, struct ExpressionStack** StackTop, struct Value* DereferenceValue)
{
    ...
    void* DerefDataLoc = VariableDereferencePointer(DereferenceValue, &DerefVal, &Offset, &DerefType, &DerefIsLValue);
...
void ExpressionPushUnsignedInt(struct ParseState* Parser, struct ExpressionStack** StackTop, unsigned int IntValue)
{
    struct Value* ValueLoc         = VariableAllocValueFromType(Parser->pc, Parser, &Parser->pc->UnsignedIntType, false, NULL, false);
    ValueLoc->Val->UnsignedInteger = IntValue;
    ExpressionStackPushValueNode(Parser, StackTop, ValueLoc);
}

void ExpressionPushLong(struct ParseState* Parser, struct ExpressionStack** StackTop, long IntValue)
{
...
void ExpressionPushUnsignedLong(struct ParseState* Parser, struct ExpressionStack** StackTop, unsigned long IntValue)
{
    struct Value* ValueLoc             = VariableAllocValueFromType(Parser->pc, Parser, &Parser->pc->UnsignedLongType, false, NULL, false);
...
{
    struct Value* ValueLoc = VariableAllocValueFromType(Parser->pc, Parser, &Parser->pc->DoubleType, false, NULL, false);
...
{
    struct Value* ValueLoc = VariableAllocValueFromType(Parser->pc, Parser, &Parser->pc->FloatType, false, NULL, false);
    ...
    struct ParseState* Parser, struct Value* ToValue, struct Value* FromValue, const char* FuncName, int ParamNo, int AllowPointerCoercion)
{
    ...
    if (FromValue->Typ == ToValue->Typ || FromValue->Typ == Parser->pc->VoidPtrType ||
             ...
             (PointedToType == FromValue->Typ->FromType->FromType || ToValue->Typ == Parser->pc->VoidPtrType))
    {
        ...
        ToValue->Val->Pointer = VariableDereferencePointer(FromValue, NULL, NULL, NULL, NULL);
    }
    ...
    else if (AllowPointerCoercion && IS_NUMERIC_COERCIBLE(FromValue))
    {
        ...
        ToValue->Val->Pointer = (void*)(unsigned long)ExpressionCoerceInteger(FromValue);
    }
    ...
    int AllowPointerCoercion)
{
    ...
    switch (DestValue->Typ->Base)
    {
        ...
        case TypeLong:
            ...
            break;
        case TypeUnsignedInt:
            DestValue->Val->UnsignedInteger = (unsigned int)ExpressionCoerceInteger(SourceValue);
            break;
        case TypeUnsignedShort:
            DestValue->Val->UnsignedShortInteger = (unsigned short)ExpressionCoerceInteger(SourceValue);
            break;
        case TypeUnsignedLong:
            DestValue->Val->UnsignedLongInteger = (unsigned long)ExpressionCoerceInteger(SourceValue);
            break;
        case TypeUnsignedChar:
            DestValue->Val->UnsignedCharacter = (unsigned char)ExpressionCoerceInteger(SourceValue);
            break;
        case TypeFloat:
        ...
        case TypeArray:
...
#endif
                    DestValue->Typ =
                        TypeGetMatching(Parser->pc, Parser, DestValue->Typ->FromType, DestValue->Typ->Base, Size, DestValue->Typ->Identifier, true);
                    VariableRealloc(Parser, DestValue, TypeSizeValue(DestValue, false));
                }
            ...
            if (DestValue->Typ->ArraySize != SourceValue->Typ->ArraySize)
                AssignFail(Parser, "from an array of size %d to one of size %d", NULL, NULL, DestValue->Typ->ArraySize, SourceValue->Typ->ArraySize,
                    FuncName, ParamNo);
        ...
        case TypeFunctionPtr:
            ...
            {
                DestValue->Val->Identifier = (char*)ExpressionCoerceInteger(SourceValue); // Might backfire in unreasonable programs
            }
            if (DestValue->Typ != SourceValue->Typ)
                AssignFail(Parser, "%t from %t", DestValue->Typ, SourceValue->Typ, 0, 0, FuncName, ParamNo);
...
void ExpressionPrefixOperator(struct ParseState* Parser, struct ExpressionStack** StackTop, enum LexToken Op, struct Value* TopValue)
{
    ...
    switch (Op)
    {
        ...
        case TokenAsterisk:
            ...
            else if (TopValue->Typ->Base == TypeFunctionPtr || (TopValue->Typ->Base == TypeArray && TopValue->Typ->FromType->Base == TypeFunctionPtr))
                ExpressionStackPushValue(Parser, StackTop, TopValue);
            else
        ...
        case TokenSizeof:
            ...
            if (Typ->FromType != NULL && Typ->FromType->Base == TypeStruct)
                Typ = Typ->FromType;
            ExpressionPushUnsignedInt(Parser, StackTop, TypeSize(Typ, Typ->ArraySize, true));
            break;

        default:
            /* an arithmetic operator */
            if (IS_FLOAT(TopValue))
            {
                ...
                {
                    case TokenPlus:
                        Result = TopValue->Val->Float;
                        break;
                    case TokenMinus:
                        Result = -TopValue->Val->Float;
                        break;
                    case TokenIncrement:
                        Result = (float)ExpressionAssignFP(Parser, TopValue, TopValue->Val->Float + 1, false);
                        break;
                    case TokenDecrement:
                        Result = (float)ExpressionAssignFP(Parser, TopValue, TopValue->Val->Float - 1, false);
                        break;
                ...
                ExpressionPushFloat(Parser, StackTop, Result);
            }
            else if (IS_DOUBLE(TopValue))
            {
                /* floating point prefix arithmetic */
                double Result = 0.0;
                ...
                {
                    case TokenPlus:
                        Result = TopValue->Val->Double;
                        break;
                    case TokenMinus:
                        Result = -TopValue->Val->Double;
                        break;
                    case TokenIncrement:
                        Result = ExpressionAssignFP(Parser, TopValue, TopValue->Val->Double + 1, false);
                        break;
                    case TokenDecrement:
                        Result = ExpressionAssignFP(Parser, TopValue, TopValue->Val->Double - 1, false);
                        break;
                ...
                ExpressionPushDouble(Parser, StackTop, Result);
            }
            else if (INTEGER_PROMOTES_TO_INT(TopValue))
            {
                int ResultInt = 0;
            ...
            else if (TopValue->Typ->Base == TypePointer)
            {
                ...
                {
                    case TokenIncrement:
                        ResultPtr = TopValue->Val->Pointer = (void*)((char*)TopValue->Val->Pointer + Size);
                        break;
                    case TokenDecrement:
                        ResultPtr = TopValue->Val->Pointer = (void*)((char*)TopValue->Val->Pointer - Size);
                        break;
                    case TokenUnaryNot:
                        /* conditionally checking a pointer's value, we only want
                            ...
                            actual value  */
                        TopValue->Val->Pointer = (void*)((char*)TopValue->Val->Pointer);
...
void ExpressionPostfixOperator(struct ParseState* Parser, struct ExpressionStack** StackTop, enum LexToken Op, struct Value* TopValue)
{
    ...
    if (IS_FLOAT(TopValue))
    {
        /* floating point postfix arithmetic */
        float ResultFP = 0.0;

        switch (Op)
        {
            case TokenIncrement:
                ResultFP = (float)ExpressionAssignFP(Parser, TopValue, TopValue->Val->Float + 1, true);
                break;
            case TokenDecrement:
                ResultFP = (float)ExpressionAssignFP(Parser, TopValue, TopValue->Val->Float - 1, true);
                break;
    ...
    else if (IS_DOUBLE(TopValue))
    {
        ...
        {
            case TokenIncrement:
                ResultFP = ExpressionAssignFP(Parser, TopValue, TopValue->Val->Double + 1, true);
                break;
            case TokenDecrement:
                ResultFP = ExpressionAssignFP(Parser, TopValue, TopValue->Val->Double - 1, true);
                break;
    ...
    struct ParseState* Parser, struct ExpressionStack** StackTop, enum LexToken Op, struct Value* BottomValue, struct Value* TopValue)
{
    ...
    if (Op == TokenLeftSquareBracket)
    {
        ...
        {
            case TypeArray:
                Result = VariableAllocValueFromExistingData(Parser, BottomValue->Typ->FromType,
                    (union AnyValue*)(&BottomValue->Val->ArrayMem[0] + TypeSize(BottomValue->Typ, ArrayIndex, true)), BottomValue->IsLValue,
                    BottomValue->LValueFrom);
                break;
            case TypePointer:
                Result = VariableAllocValueFromExistingData(Parser, BottomValue->Typ->FromType,
                    (union AnyValue*)((char*)BottomValue->Val->Pointer + TypeSize(BottomValue->Typ->FromType, 0, true) * ArrayIndex),
    ...
    else if ((IS_FP(TopValue) && IS_NUMERIC_COERCIBLE(BottomValue)) || (IS_NUMERIC_COERCIBLE(TopValue) && IS_FP(BottomValue)))
    {
        int ResultIsInt = false;
        double ResultFP;
        // Doubles and floats will both unconditionally fit inside of a double
        if (IS_FP(BottomValue) && IS_FP(TopValue))
        {
            double BottomFP = ExpressionCoerceDouble(BottomValue);
            double TopFP    = ExpressionCoerceDouble(TopValue);

            switch (Op)
            {
                case TokenAssign:
                    ResultFP = ExpressionAssignFP(Parser, BottomValue, TopFP, false);
                    break;
                case TokenAddAssign:
                    ResultFP = ExpressionAssignFP(Parser, BottomValue, BottomFP + TopFP, false);
                    break;
        ...
        // But longs and unsigned longs will not
        else if (IS_FP(BottomValue))
        {
            double BottomFP = ExpressionCoerceDouble(BottomValue);
            if (IS_INTEGER_UNSIGNED(TopValue))
            {
                unsigned long TopInt = (unsigned long)ExpressionCoerceInteger(TopValue);
                switch (Op)
                {
                    case TokenAssign:
                        ResultInt = ExpressionAssignFP(Parser, BottomValue, TopInt, false);
                        break;
                    case TokenAddAssign:
                        ResultInt = ExpressionAssignFP(Parser, BottomValue, BottomFP + TopInt, false);
                        break;
                    case TokenSubtractAssign:
                        ResultInt = ExpressionAssignFP(Parser, BottomValue, BottomFP - TopInt, false);
                        break;
        ...
        {
            double TopFP = ExpressionCoerceDouble(TopValue);
            if (IS_INTEGER_UNSIGNED(BottomValue))
            {
                ...
                switch (Op)
                {
                    case TokenAssign:
                        ...
                        break;
                    case TokenAddAssign:
                        ResultInt = ExpressionAssignInt(Parser, BottomValue, BottomInt + TopFP, false);
                        break;
                    case TokenSubtractAssign:
                        ResultInt = ExpressionAssignInt(Parser, BottomValue, BottomInt - TopFP, false);
                        break;
        ...
        if (ResultIsInt)
            ExpressionPushInt(Parser, StackTop, ResultInt);
        else if ((IS_FLOAT(TopValue) || IS_INTEGER_NUMERIC(TopValue)) && (IS_FLOAT(BottomValue) || IS_INTEGER_NUMERIC(BottomValue)))
    ...
    else if (Op == TokenCast)
    {
        ...
        /* XXX - possible bug if the destination type takes more than sizeof(struct Value) + sizeof(struct
                                                  ValueType *) */
            ...
            switch (TopOperatorNode->Order)
            {
                ...
                case OrderInfix:
                    ...
                    if (TopValue != NULL)
                    {
                        ...
                        HeapPopStack(Parser->pc, NULL, sizeof(struct ExpressionStack));
                        HeapPopStack(
                            Parser->pc, BottomValue, sizeof(struct ExpressionStack) + sizeof(struct Value) + TypeStackSizeValue(BottomValue));
...
void ExpressionStackPushOperator(
    struct ParseState* Parser, struct ExpressionStack** StackTop, enum OperatorOrder Order, enum LexToken Token, int Precedence)
{
...
void ExpressionGetStructElement(struct ParseState* Parser, struct ExpressionStack** StackTop, enum LexToken Token)
{
    ...
    if (Parser->Mode == RunModeRun)
    {
        ...
        if (Token == TokenArrow)
            DerefDataLoc = VariableDereferencePointer(ParamVal, &StructVal, NULL, &StructType, NULL);

        if (StructType->Base != TypeStruct && StructType->Base != TypeUnion)
            ProgramFail(Parser, "can't use '%s' on something that's not a struct or union %s : it's a %t", (Token == TokenDot) ? "." : "->",
        ...
        *StackTop = (*StackTop)->Next;
        ...
        Result = VariableAllocValueFromExistingData(
            Parser, MemberValue->Typ, (void*)(DerefDataLoc + MemberValue->Val->Integer), true, (StructVal != NULL) ? StructVal->LValueFrom : NULL);
        ExpressionStackPushValueNode(Parser, StackTop, Result);
    }
}
...
int ExpressionParse(struct ParseState* Parser, struct Value** Result)
{
    ...
    do
    {
        ...
        if ((((int)Token > TokenComma && (int)Token <= (int)TokenOpenBracket) || (Token == TokenCloseBracket && BracketPrecedence != 0)) &&
            ...
            else
            {
                ...
                else if (OperatorPrecedence[(int)Token].InfixPrecedence != 0)
                {
                    ...
                    if (Token == TokenOpenBracket)
                    {
                        ...
                        ExpressionParseFunctionCall(Parser, &StackTop, RunIt ? StackTop->Next->Val->Val->Identifier : Parser->pc->StrEmpty, RunIt);
                        PrefixState = false;
                    }
                }
                else
        ...
        else if (Token == TokenIdentifier)
        {
            ...
            else
            {
                if (Parser->Mode == RunModeRun /* && Precedence < IgnorePrecedence */)
                {
                    ...
                    else if (VariableValue->Typ == &Parser->pc->FunctionType) // a function pointer identifier
                    {
            ...
            if (RunIt)
            {
                ...
                else if (FuncParser.Mode == RunModeGoto)
                {
                    do
                    {
                        ...
                        ParseStatement(&FuncParser, false);
                    } while (FuncParser.Mode == RunModeGoto && FuncParser.FreshGotoSearch == false);
